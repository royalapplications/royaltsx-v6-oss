...
//        session.useMetal = NO;
        ...
        if (session.view) {
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   selector:@selector(sessionViewDidEndLiveResize:)
...
- (void)sendKeyCode:(unsigned short)keyCode characters:(NSString*)characters modifierFlags:(NSUInteger)modifierFlags isDown:(BOOL)down
{
    if (!self.session ||
        !self.session.textview) {
        return;
    }
    
    __weak PTYTextView* weakTextView = self.session.textview;
    __weak NSString* weakCharacters = characters != nil ? characters : @"";
    ...
    runOnMainQueueWithoutDeadlocking(^{
        ...
        @autoreleasepool {
            NSWindow* window = weakTextView.window;
            NSInteger windowNumber = window ? window.windowNumber : 0;
            
            NSEventType eventType = down ? NSEventTypeKeyDown : NSEventTypeKeyUp;
            ...
            NSEvent* ev = [NSEvent keyEventWithType:eventType
                        ...
                        charactersIgnoringModifiers:weakCharacters.lowercaseString
                                          isARepeat:NO
                                            keyCode:keyCode];
            
            if (weakTextView) {
                [weakTextView keyDown:ev];
            }
        }
    });
}
...
- (void)sessionViewDidEndLiveResize:(NSNotification*)aNotification
{
    if (_isTerminated ||
        _terminateInProgress ||
        !self.session ||
...
{
//    NSLog(@"fitSessionToCurrentViewSize: 01");
    ...
    if (aSession.view.inLiveResize) {
        return NO;
    }
    
//    NSLog(@"fitSessionToCurrentViewSize: 02");
...
- (VT100GridSize)sessionTmuxSizeWithProfile:(NSDictionary *)profile {
    NSSize frameSize = self.session.textview.frame.size;
    PTYSession *anySession = self.session;
    
    NSSize contentSize = [NSScrollView contentSizeForFrameSize:frameSize
                                       horizontalScrollerClass:nil
                                         verticalScrollerClass:self.scrollbarShouldBeVisible ? [[anySession.view.scrollview verticalScroller] class] : nil
...
- (ITPSMTabBarControl*)tabBarControl { return nil; }
