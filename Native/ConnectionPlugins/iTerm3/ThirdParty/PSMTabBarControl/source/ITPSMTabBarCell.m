...
@interface ITPSMTabBarCell()<ITPSMProgressIndicatorDelegate>
- (NSView<ITPSMTabBarControlProtocol> *)psmTabControlView;
@end
...
@property(nonatomic, assign) ITPSMTabBarCell *cell;
- (instancetype)initWithCell:(ITPSMTabBarCell *)cell role:(NSString *)role;
@end
...
- (NSRect)accessibilityFrame {
    ITPSMTabBarCell *cell = self.cell;
...
- (NSRect)accessibilityFrame {
    ITPSMTabBarCell *cell = self.cell;
    NSView<ITPSMTabBarControlProtocol> *controlView = cell.psmTabControlView;
...
@implementation ITPSMTabBarCell  {
    NSSize _stringSize;
    ITPSMProgressIndicator *_indicator;
...
- (id)initWithControlView:(ITPSMTabBarControl *)controlView {
    ...
    if (self) {
        ...
        _indicator = [[ITPSMProgressIndicator alloc] initWithFrame:NSMakeRect(0,
                                                                            0,
...
- (id)initPlaceholderWithFrame:(NSRect)frame
                      expanded:(BOOL)value
                 inControlView:(ITPSMTabBarControl *)controlView {
    self = [super init];
    if (self) {
        [self setControlView:controlView];
        _isPlaceholder = YES;
...
- (NSView<ITPSMTabBarControlProtocol> *)psmTabControlView {
    return (NSView<ITPSMTabBarControlProtocol> *)[self controlView];
}
...
- (NSAttributedString *)attributedStringValue {
    id<ITPSMTabBarControlProtocol> control = [self psmTabControlView];
    id <ITPSMTabStyle> tabStyle = [control style];
    return [tabStyle attributedStringValueForTabCell:self];
}

- (ITPSMProgressIndicator *)indicator {
    return _indicator;
}
...
- (NSImage *)dragImage {
    ...
    if (![[self indicator] isHidden]) {
        ...
        NSImage *piImage = [[NSImage alloc] initByReferencingFile:[[ITPSMTabBarControl bundle] pathForImageResource:@"pi"]];
        [returnImage lockFocus];
...
- (CGFloat)highlightAnimationDuration {
    if ([(ITPSMTabBarControl *)self.controlView orientation] == PSMTabBarHorizontalOrientation) {
        return 0.2;
    } else {
        return 0;
    }
...
- (void)progressIndicatorNeedsUpdate {
    return [(ITPSMTabBarControl *)self.controlView progressIndicatorNeedsUpdate];
}
