...
@interface ITPSMTabBarCell(ITPSMYosemiteTabStyle)
...
@implementation ITPSMTabBarCell(ITPSMYosemiteTabStyle)
...
@implementation ITPSMYosemiteTabStyle {
...
- (id)init {
    if ((self = [super init]))  {
        ...
        _addTabButtonImage = [[NSImage alloc] initByReferencingFile:[[ITPSMTabBarControl bundle] pathForImageResource:@"TabNewMetal"]];
        _addTabButtonPressedImage = [[NSImage alloc] initByReferencingFile:[[ITPSMTabBarControl bundle] pathForImageResource:@"TabNewMetalPressed"]];
        _addTabButtonRolloverImage = [[NSImage alloc] initByReferencingFile:[[ITPSMTabBarControl bundle] pathForImageResource:@"TabNewMetalRollover"]];
    }
    return self;
}
...
- (NSRect)dragRectForTabCell:(ITPSMTabBarCell *)cell
...
- (NSRect)iconRectForTabCell:(ITPSMTabBarCell *)cell {
    ...
    result.origin.x = minX - kITPSMTabBarCellIconPadding - kPSMTabBarIconWidth;
...
- (NSRect)indicatorRectForTabCell:(ITPSMTabBarCell *)cell {
    ...
    result.origin.x = minX - kITPSMTabBarCellIconPadding - kPSMTabBarIndicatorWidth;
    result.origin.y = cellFrame.origin.y + floor((cellFrame.size.height - result.size.height) / 2.0);
...
- (CGFloat)widthOfLeftMatterInCell:(ITPSMTabBarCell *)cell {
    ...
    // close button?
    resultWidth += [_closeButton size].width + kITPSMTabBarCellPadding;

    // icon?
    if ([cell hasIcon]) {
        resultWidth += kPSMTabBarIconWidth + kITPSMTabBarCellIconPadding;
    }
    return resultWidth;
}

- (CGFloat)widthOfRightMatterInCell:(ITPSMTabBarCell *)cell {
    CGFloat resultWidth = 0;
    // object counter?
    if ([cell count] > 0) {
        resultWidth += [self objectCounterRectForTabCell:cell].size.width + kITPSMTabBarCellPadding;
    } else {
        resultWidth += [_closeButton size].width + kITPSMTabBarCellPadding;
    }
    ...
    if ([[cell indicator] isHidden] == NO) {
        resultWidth += kITPSMTabBarCellPadding + kPSMTabBarIndicatorWidth;
    }
...
- (CGFloat)widthOfAttributedStringInCell:(ITPSMTabBarCell *)cell {
...
- (NSColor *)textColorForCell:(ITPSMTabBarCell *)cell {
    NSColor *textColor;
    ...
    if ([self anyTabHasColor]) {
        ...
        if (selected) {
            ...
            CGFloat prominence = [[_tabBar.delegate tabView:_tabBar valueOfOption:ITPSMTabBarControlOptionColoredUnselectedTabTextProminence] doubleValue];
...
- (BOOL)anyTabHasColor {
    return [_tabBar.cells indexOfObjectPassingTest:^BOOL(ITPSMTabBarCell * _Nonnull cell, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)drawPostHocDecorationsOnSelectedCell:(ITPSMTabBarCell *)cell
    ...
    if (self.anyTabHasColor) {
        ...
        NSColor *outerColor;
        NSColor *innerColor;
        NSNumber *strengthNumber = [bar.delegate tabView:bar valueOfOption:ITPSMTabBarControlOptionColoredSelectedTabOutlineStrength] ?: @0.5;
...
- (void)drawInteriorWithTabCell:(ITPSMTabBarCell *)cell
    ...
    if ([cell hasCloseButton]) {
        if (cell.isCloseButtonSuppressed && _orientation == PSMTabBarHorizontalOrientation) {
            ...
            // [(reserved)long-------------label   ]
            reservedSpace = closeButtonSize.width + kITPSMTabBarCellPadding;
        } else {
            labelPosition += closeButtonSize.width + kITPSMTabBarCellPadding;
        }
    }
    ...
    if (attributedString.length > 0) {
        ...
        labelRect.size.width = cellFrame.size.width - (labelRect.origin.x - cellFrame.origin.x) - kITPSMTabBarCellPadding;
        if ([cell hasIcon]) {
            // Reduce size of label if there is an icon or activity indicator
            labelRect.size.width -= iconRect.size.width + kITPSMTabBarCellIconPadding;
        } else if (![[cell indicator] isHidden]) {
            labelRect.size.width -= cell.indicator.frame.size.width + kITPSMTabBarCellIconPadding;
        }
        labelRect.size.height = cellFrame.size.height;

        if ([cell count] > 0) {
            labelRect.size.width -= ([self objectCounterRectForTabCell:cell].size.width + kITPSMTabBarCellPadding);
        }
...
- (void)drawTabBar:(ITPSMTabBarControl *)bar
            inRect:(NSRect)rect
        horizontal:(BOOL)horizontal {
    ...
    if (![bar tabView]){
        ...
        NSString *contents = @"ITPSMTabBarControl";
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (ITPSMTabBarCell *cell in [bar cells]) {
