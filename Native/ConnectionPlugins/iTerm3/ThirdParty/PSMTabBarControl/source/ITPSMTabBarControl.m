...
#import "ITPSMOverflowPopUpButton.h"
#import "ITPSMRolloverButton.h"
#import "ITPSMTabStyle.h"
#import "ITPSMYosemiteTabStyle.h"
#import "ITPSMTabDragAssistant.h"
#import "PTYTask.h"
...
const CGFloat kITPSMTabBarControlHeight = 24;
const CGFloat kSPMTabBarCellInternalXMargin = 6;

const CGFloat kITPSMTabBarCellPadding = 4;
const CGFloat kITPSMTabBarCellIconPadding = 0;
// fixed size objects
...
ITPSMTabBarControlOptionKey ITPSMTabBarControlOptionColoredSelectedTabOutlineStrength = @"ITPSMTabBarControlOptionColoredSelectedTabOutlineStrength";
ITPSMTabBarControlOptionKey ITPSMTabBarControlOptionColoredUnselectedTabTextProminence = @"ITPSMTabBarControlOptionColoredUnselectedTabTextProminence";

@interface ITPSMTabBarControl ()<ITPSMTabBarControlProtocol>
@end

@implementation ITPSMTabBarControl {
    // control basics
    NSMutableArray *_cells; // the cells that draw the tabs
    ITPSMOverflowPopUpButton *_overflowPopUpButton; // for too many tabs
    ITPSMRolloverButton *_addTabButton;
...
+ (NSBundle *)bundle {
    ...
    if (!bundle) bundle = [NSBundle bundleForClass:[ITPSMTabBarControl class]];
    return bundle;
}
...
- (NSRect)genericCellRect {
    ...
    aRect.size.height = kITPSMTabBarControlHeight;
    return aRect;
}
...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        _style = [[ITPSMYosemiteTabStyle alloc] init];
        ...
        _overflowPopUpButton = [[ITPSMOverflowPopUpButton alloc] initWithFrame:overflowButtonRect pullsDown:YES];
        ...
        _addTabButton = [[ITPSMRolloverButton alloc] initWithFrame:addTabButtonRect];
        if (_addTabButton){
...
- (void)dealloc {
    ...
    for (ITPSMTabBarCell *cell in temp) {
        [cell retain];
...
- (void)sanityCheck:(NSString *)callsite force:(BOOL)force {
    if (!force && [[ITPSMTabDragAssistant sharedDragAssistant] isDragging]) {
    ...
    if (self.tabView.tabViewItems.count != self.cells.count) {
        ...
        for (NSInteger i = 0; i < self.cells.count; i++) {
            ...
            ITPSMTabBarCell *cell = self.cells[i];
...
- (void)setDelegate:(id<ITPSMTabBarControlDelegate>)object {
    _delegate = object;
...
- (void)setStyle:(id <ITPSMTabStyle>)newStyle {
    [_style autorelease];
...
- (NSLineBreakMode)truncationStyle {
    ...
    for (ITPSMTabBarCell *cell in _cells) {
...
- (void)addTabViewItem:(NSTabViewItem *)item atIndex:(NSUInteger)i {
    // create cell
    ITPSMTabBarCell *cell = [[ITPSMTabBarCell alloc] initWithControlView:self];
...
{
    for (ITPSMTabBarCell *cell in [self cells]) {
        [cell setIsLast:NO];
    }
...
- (void)update:(BOOL)animate {
    ...
    // the refresh.  This method is not called during drag and drop. See the ITPSMTabDragAssistant's
...
- (void)_animateCells:(NSTimer *)timer {
    ...
    if ([_cells count] > 0) {
        ...
        for (i = 0; i < [_cells count]; i++) {
            ITPSMTabBarCell *currentCell = [_cells objectAtIndex:i];
...
- (NSMenu *)_setupCells:(NSArray *)newValues {
    ...
    for (i = 0; i < cellCount; i++) {
        ITPSMTabBarCell *cell = [_cells objectAtIndex:i];
        int tabState = 0;
        if (i < numberOfVisibleCells) {
            // set cell frame
            if ([self orientation] == PSMTabBarHorizontalOrientation) {
            ...
            if ([[cell representedObject] isEqualTo:[_tabView selectedTabViewItem]]) {
                ...
                // previous cell
                if (i > 0) {
                    [[_cells objectAtIndex:i-1] setTabState:([(ITPSMTabBarCell *)[_cells objectAtIndex:i-1] tabState] | PSMTab_RightIsSelectedMask)];
                }
                // next cell - see below
            } else {
...
- (void)_setupOverflowMenu:(NSMenu *)overflowMenu
{
    ...
    cellRect.size.height = kITPSMTabBarControlHeight;
    ...
    if ([self orientation] == PSMTabBarHorizontalOrientation) {
        ...
        cellRect.origin.x = 0;
        cellRect.origin.y = [self frame].size.height - kITPSMTabBarControlHeight;
        cellRect.size.width = [self frame].size.width;
    }
...
- (void)mouseDown:(NSEvent *)theEvent
{
    ...
    NSRect cellFrame;
    ITPSMTabBarCell *cell = [self cellForPoint:mousePt cellFrame:&cellFrame];
    if (cell) {
...
- (void)mouseDragged:(NSEvent *)theEvent
{
    ...
    ITPSMTabBarCell *cell = [self cellForPoint:trackingStartPoint cellFrame:&cellFrame];
    if (cell) {
        ...
        if (distance >= self.minimumTabDragDistance && !_didDrag && ![[ITPSMTabDragAssistant sharedDragAssistant] isDragging] &&
            ...
            [[ITPSMTabDragAssistant sharedDragAssistant] startDraggingCell:cell fromTabBar:self withMouseDownEvent:[self lastMouseDownEvent]];
        }
    }
}
...
- (void)otherMouseUp:(NSEvent *)theEvent
{
    ...
    if ([theEvent buttonNumber] == 2 && !_resizing) {
        ...
        ITPSMTabBarCell *mouseDownCell = [self cellForPoint:[self convertPoint:[[self lastMiddleMouseDownEvent] locationInWindow] fromView:nil]
...
- (void)mouseUp:(NSEvent *)theEvent
{
    ...
    if (_resizing) {
        ...
        ITPSMTabBarCell *cell = [self cellForPoint:mousePt cellFrame:&cellFrame];
        ITPSMTabBarCell *mouseDownCell = [self cellForPoint:[self convertPoint:[[self lastMouseDownEvent] locationInWindow] fromView:nil] cellFrame:&mouseDownCellFrame];
        if (cell) {
...
- (void)draggingSession:(NSDraggingSession *)session willBeginAtPoint:(NSPoint)screenPoint {
    [[ITPSMTabDragAssistant sharedDragAssistant] draggingBeganAt:screenPoint];
}

- (void)draggingSession:(NSDraggingSession *)session movedToPoint:(NSPoint)screenPoint {
    [[ITPSMTabDragAssistant sharedDragAssistant] draggingMovedTo:screenPoint];
}
    ...
    if ([[[sender draggingPasteboard] types] indexOfObject:@"com.iterm2.psm.controlitem"] != NSNotFound) {
        if ([[self delegate] respondsToSelector:@selector(tabView:shouldDropTabViewItem:inTabBar:)] &&
            ![[self delegate] tabView:[[sender draggingSource] tabView]
                shouldDropTabViewItem:[[[ITPSMTabDragAssistant sharedDragAssistant] draggedCell] representedObject]
                             inTabBar:self]) {
            return NSDragOperationNone;
        }

        [[ITPSMTabDragAssistant sharedDragAssistant] draggingEnteredTabBar:self atPoint:[self convertPoint:[sender draggingLocation] fromView:nil]];
        ...
        if (op != NSDragOperationNone) {
            [[ITPSMTabDragAssistant sharedDragAssistant] startAnimationWithOrientation:_orientation width:_cellOptimumWidth];
            [[ITPSMTabDragAssistant sharedDragAssistant] draggingEnteredTabBar:self atPoint:[self convertPoint:[sender draggingLocation] fromView:nil]];
        }
        return op;
    } else {
...
- (NSDragOperation)draggingUpdated:(id <NSDraggingInfo>)sender {
    ITPSMTabBarCell *cell = [self cellForPoint:[self convertPoint:[sender draggingLocation] fromView:nil] cellFrame:nil];
    ...
    if ([[[sender draggingPasteboard] types] indexOfObject:@"com.iterm2.psm.controlitem"] != NSNotFound) {
        ...
        [[ITPSMTabDragAssistant sharedDragAssistant] draggingUpdatedInTabBar:self atPoint:[self convertPoint:[sender draggingLocation] fromView:nil]];
        return NSDragOperationMove;
...
- (void)draggingExited:(id <NSDraggingInfo>)sender {
    [[ITPSMTabDragAssistant sharedDragAssistant] draggingExitedTabBar:self];
}
...
- (BOOL)prepareForDragOperation:(id <NSDraggingInfo>)sender {
    ...
    return badType ||
           [[ITPSMTabDragAssistant sharedDragAssistant] destinationTabBar] != nil;
}
...
- (BOOL)performDragOperation:(id <NSDraggingInfo>)sender {
    ...
    if ([[[sender draggingPasteboard] types] indexOfObject:@"com.iterm2.psm.controlitem"] != NSNotFound ||
        ...
        [[ITPSMTabDragAssistant sharedDragAssistant] performDragOperation:sender];
...
- (void)draggingSession:(NSDraggingSession *)session endedAtPoint:(NSPoint)aPoint operation:(NSDragOperation)operation {
    ...
    if (operation != NSDragOperationNone) {
        [self removeTabForCell:[[ITPSMTabDragAssistant sharedDragAssistant] draggedCell]];
        [[ITPSMTabDragAssistant sharedDragAssistant] finishDrag];
    } else {
        [[ITPSMTabDragAssistant sharedDragAssistant] draggedImageEndedAt:aPoint operation:operation];
    }
}
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)aTabView {
    ...
    for (ITPSMTabBarCell *cell in cellsToRemove) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    if ([aCoder allowsKeyedCoding]) {
        ...
        [aCoder encodeObject:_overflowPopUpButton forKey:@"ITPSMOverflowPopUpButton"];
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        if ([aDecoder allowsKeyedCoding]) {
            ...
            _overflowPopUpButton = [[aDecoder decodeObjectForKey:@"ITPSMOverflowPopUpButton"] retain];
...
- (ITPSMTabBarCell *)cellWithIdentifier:(id)identifier {
    for (ITPSMTabBarCell *cell in _cells) {
...
- (void)setIsProcessing:(BOOL)isProcessing forTabWithIdentifier:(id)identifier {
    ITPSMTabBarCell *cell = [self cellWithIdentifier:identifier];
...
- (NSArray*)accessibilityChildren {
    ...
    for (ITPSMTabBarCell *cell in [_cells objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, [self numberOfVisibleTabs])]]) {
...
#pragma mark - ITPSMProgressIndicatorDelegate
