...
@interface ITPSMTabDragAssistant()
@property (nonatomic, retain) ITPSMTabBarControl *sourceTabBar;
@property (nonatomic, retain) ITPSMTabBarControl *destinationTabBar;
@property (nonatomic, retain) ITPSMTabBarCell *draggedCell;
@property (nonatomic) int draggedCellIndex;   // for snap back
@property (nonatomic) BOOL isDragging;
@property (nonatomic) NSPoint currentMouseLoc;
@property (nonatomic, retain) ITPSMTabBarCell *targetCell;
...
@implementation ITPSMTabDragAssistant {
    ITPSMTabBarControl *_destinationTabBar;
    NSMutableSet *_participatingTabBars;

    // Support for dragging into new windows
    ITPSMTabDragWindow *_dragTabWindow, *_dragViewWindow;
...
+ (ITPSMTabDragAssistant *)sharedDragAssistant {
    ...
    static ITPSMTabDragAssistant *sharedDragAssistant = nil;
    dispatch_once(&onceToken, ^{
        sharedDragAssistant = [[ITPSMTabDragAssistant alloc] init];
    });
...
- (void)startAnimationWithOrientation:(PSMTabBarOrientation)orientation width:(CGFloat)width {
    if (_sineCurveWidths.count == 0) {
        [self addSineCurveWidthsWithOrientation:orientation
                                           size:NSMakeSize(width, kITPSMTabBarControlHeight)];
    }
    [self startAnimation];
}

- (void)startDraggingCell:(ITPSMTabBarCell *)cell
               fromTabBar:(ITPSMTabBarControl *)control
    ...
    _dragTabWindow = [[ITPSMTabDragWindow dragWindowWithTabBarCell:cell
                                                           image:dragImage
                                                       styleMask:NSWindowStyleMaskBorderless] retain];
    [_dragTabWindow setAlphaValue:kITPSMTabDragWindowAlpha];
...
- (void)draggingExitedTabBar:(ITPSMTabBarControl *)control {
    ...
    if (_fadeTimer) {
        ...
        if (!_dragViewWindow) {
            ...
            if ([control delegate] &&
                ...
                if ([control orientation] == PSMTabBarHorizontalOrientation) {
                    drawPoint.y += kITPSMTabBarControlHeight - [tabImage size].height;
                    _dragWindowOffset.height -= kITPSMTabBarControlHeight - [tabImage size].height;
                } else {
            ...
            if (self.sourceTabBar.tabLocation == PSMTab_LeftTab) {
                _dragWindowOffset.height += kITPSMTabBarControlHeight;
            } else if (!(styleMask & NSWindowStyleMaskTitled)) {
                _dragWindowOffset.height += 22;
            }

            _dragViewWindow = [[ITPSMTabDragWindow dragWindowWithTabBarCell:[self draggedCell]
                                                                    image:viewImage
        ...
        if ([[[self sourceTabBar] tabView] numberOfTabViewItems] == 1 &&
            ...
            [_dragViewWindow setAlphaValue:kITPSMTabDragWindowAlpha];
        } else {
            _fadeTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 / 30.0
                                                          target:self
                                                        selector:@selector(fadeInDragWindow:)
...
- (void)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if (![self draggedCell]) {
        ...
        ITPSMTabBarCell *before = nil;
        if (destinationIndex > 0) {
            before = [cells objectAtIndex:destinationIndex - 1];
        }
        ITPSMTabBarCell *after = nil;
    ...
    ITPSMTabBarControl *destination = [[[self destinationTabBar] retain] autorelease];
    ITPSMTabBarControl *source = [[[self sourceTabBar] retain] autorelease];
...
- (void)draggedImageEndedAt:(NSPoint)aPoint operation:(NSDragOperation)operation {
    if ([self isDragging]) {  // means there was not a successful drop (performDragOperation)
        ...
        if ([self destinationTabBar] == nil &&
            ...
            ITPSMTabBarControl *control = [sourceDelegate tabView:[[self sourceTabBar] tabView]
...
- (void)finishDrag {
    ...
    [self setDestinationTabBar:nil];
    for (ITPSMTabBarControl *tabBar in _participatingTabBars) {
...
- (void)fadeInDragWindow:(NSTimer *)timer {
    ...
    if (value >= kITPSMTabDragWindowAlpha || _dragTabWindow == nil) {
        [timer invalidate];
        _fadeTimer = nil;
    } else {
...
- (void)fadeOutDragWindow:(NSTimer *)timer {
    ...
    if (value <= 0.0) {
        ...
        if ([_dragTabWindow alphaValue] < kITPSMTabDragWindowAlpha) {
            [_dragTabWindow setAlphaValue:[_dragTabWindow alphaValue] + 0.15];
        }
        [_dragViewWindow setAlphaValue:value - 0.15];
    }
...
- (void)animateDrag:(NSTimer *)timer {
    ...
    for (int i = 0; i < [objects count]; ++i) {
        ITPSMTabBarControl* tabBar = [objects objectAtIndex:i];
...
- (void)calculateDragAnimationForTabBar:(ITPSMTabBarControl *)control {
    ...
    if ([self destinationTabBar] == control) {
        ...
        if (mouseLoc.x < [[control style] leftMarginForTabBarControl]) {
            ...
            NSRect overCellRect;
            ITPSMTabBarCell *overCell = [control cellForPoint:mouseLoc cellFrame:&overCellRect];
            if (overCell) {
    ...
    for (i = 0; i < cellCount; i++) {
        ITPSMTabBarCell *cell = [cells objectAtIndex:i];
...
- (void)distributePlaceholdersInTabBar:(ITPSMTabBarControl *)control
                       withDraggedCell:(ITPSMTabBarCell *)cell {
    ...
    ITPSMTabBarCell *pc = [[[ITPSMTabBarCell alloc] initPlaceholderWithFrame:[[self draggedCell] frame] expanded:YES inControlView:control] autorelease];
...
- (void)distributePlaceholdersInTabBar:(ITPSMTabBarControl *)control {
    ...
    ITPSMTabBarCell *draggedCell = [self draggedCell];
    ...
    for (i = 0; i < numVisibleTabs; i++) {
        ITPSMTabBarCell *pc = [[[ITPSMTabBarCell alloc] initPlaceholderWithFrame:draggedCellFrame expanded:NO inControlView:control] autorelease];
...
- (void)removeAllPlaceholdersFromTabBar:(ITPSMTabBarControl *)control {
    ...
    for (i = (cellCount - 1); i >= 0; i--) {
        ITPSMTabBarCell *cell = [[control cells] objectAtIndex:i];
        if ([cell isPlaceholder]) {
