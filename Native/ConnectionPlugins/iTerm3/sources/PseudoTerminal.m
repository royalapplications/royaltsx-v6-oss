...
#import "ITPSMDarkTabStyle.h"
#import "ITPSMDarkHighContrastTabStyle.h"
#import "ITPSMLightHighContrastTabStyle.h"
...
- (BOOL)tabView:(NSTabView*)aTabView
    shouldDragTabViewItem:(NSTabViewItem *)tabViewItem
               fromTabBar:(ITPSMTabBarControl *)tabBarControl
{
    return YES;
}
...
- (BOOL)tabView:(NSTabView*)aTabView
    shouldDropTabViewItem:(NSTabViewItem *)tabViewItem
                 inTabBar:(ITPSMTabBarControl *)aTabBarControl {
              ...
              styleMask:(unsigned int *)styleMask
{
    ...
    if (tabViewItem == [aTabView selectedTabViewItem]) {
        ...
        ITPSMTabBarControl *control = (ITPSMTabBarControl *)[aTabView delegate];
        [(id <ITPSMTabStyle>)[control style] drawBackgroundInRect:tabFrame
                                                          color:nil
        ...
        offset->width = [(id <ITPSMTabStyle>)[_contentView.tabBarControl style] leftMarginForTabBarControl];
        if ([iTermPreferences intForKey:kPreferenceKeyTabPosition] == PSMTab_TopTab) {
...
- (ITPSMTabBarControl *)tabView:(NSTabView *)aTabView
    ...
    if (@available(macOS 10.13, *)) {
        ...
        if (backgroundColor == nil && [iTermAdvancedSettingsModel darkThemeHasBlackTitlebar]) {
            switch ([iTermPreferences intForKey:kPreferenceKeyTabStyle]) {
                ...
                case TAB_STYLE_DARK:
                case TAB_STYLE_DARK_HIGH_CONTRAST:  // fall through
                    backgroundColor = [ITPSMDarkTabStyle tabBarColor];
                    darkAppearance = YES;
                    break;
            }
        }
...
- (id)tabView:(ITPSMTabBarControl *)tabView valueOfOption:(ITPSMTabBarControlOptionKey)option {
    if ([option isEqualToString:ITPSMTabBarControlOptionColoredSelectedTabOutlineStrength]) {
        return @([iTermAdvancedSettingsModel coloredSelectedTabOutlineStrength]);
    } else if ([option isEqualToString:ITPSMTabBarControlOptionColoredUnselectedTabTextProminence]) {
...
- (void)updateTabBarStyle {
    ...
    switch ([self.window.effectiveAppearance it_tabStyle:preferredStyle]) {
        ...
        case TAB_STYLE_COUNT:
            assert(NO);
        case TAB_STYLE_LIGHT:
            style = [[[ITPSMYosemiteTabStyle alloc] init] autorelease];
            break;
        case TAB_STYLE_DARK:
            style = [[[ITPSMDarkTabStyle alloc] init] autorelease];
            break;
        case TAB_STYLE_LIGHT_HIGH_CONTRAST:
            style = [[[ITPSMLightHighContrastTabStyle alloc] init] autorelease];
            break;
        case TAB_STYLE_DARK_HIGH_CONTRAST:
            style = [[[ITPSMDarkHighContrastTabStyle alloc] init] autorelease];
            break;
    }
...
- (ITPSMTabBarControl*)tabBarControl
{
