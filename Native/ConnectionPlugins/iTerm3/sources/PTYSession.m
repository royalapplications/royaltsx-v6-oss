...
- (void)startProgram:(NSString *)command
    ...
    // iTermLib Edit: Backported on 22.11.2019
...
- (void)_maybeWarnAboutShortLivedSessions
{
    ...
    if ([[NSDate date] timeIntervalSinceDate:_creationDate] < [iTermAdvancedSettingsModel shortLivedSessionDuration]) {
        ...
        // iTermLib Edit: If the user chooses to supress these warnings, save the info without the profile as this will be generated on-demand
        ...
        NSString* theKey = @"NeverWarnAboutShortLivedSessions";
...
// iTermLib Edit:
- (void)clearBufferAfterEnteringPassword
{
    [self performSelector:@selector(clearBuffer) withObject:nil afterDelay:0.1];
}

// iTermLib Edit:
- (void)enterPassword:(NSString *)password
{
    [self enterPassword:password showNoPromptAndClearBufferAfterEnteringPassword:NO];
}

// iTermLib Edit:
- (void)enterPassword:(NSString *)password showNoPromptAndClearBufferAfterEnteringPassword:(BOOL)showNoPromptAndClearBufferAfterEnteringPassword {
    ...
    if (backspace) {
        ...
        // iTermLib Edit:
        if (showNoPromptAndClearBufferAfterEnteringPassword) {
            [self performSelector:@selector(enterPasswordIfEchoProbeOkNoPromptAndClearScreenAfterwards:)
                       withObject:password
        ...
        // iTermLib Edit:
        if (showNoPromptAndClearBufferAfterEnteringPassword) {
            [self clearBufferAfterEnteringPassword];
        }
    }
}

// iTermLib Edit:
- (void)enterPasswordIfEchoProbeOkNoPromptAndClearScreenAfterwards:(NSString *)password {
    BOOL ok = NO;
    @synchronized (self) {
        switch (_echoProbeState) {
            case iTermEchoProbeWaiting:
