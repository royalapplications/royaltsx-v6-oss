...
@implementation PTYTab {
    ...
    // The tab number that is observed by ITPSMTabBarControl.
    int objectCount_;

    // The icon to display in the tab. Observed by ITPSMTabBarControl.
    NSImage* icon_;

    // Whether the session is "busy". Observed by ITPSMTabBarControl.
    BOOL isProcessing_;
...
// This is KVO-observed by ITPSMTabBarControl and determines whether the activity indicator is visible.
- (BOOL)isProcessing {
...
- (NSImage*)image:(BOOL)withSpaceForFrame {
    ...
    if (withSpaceForFrame) {
        ...
        ITPSMTabBarControl *control = (ITPSMTabBarControl *)[[realParentWindow_ tabView] delegate];
...
- (void)setLabelAttributesForIdleTab {
    ...
    if ([self isProcessing]) {
        [self setIsProcessing:NO];  // This triggers KVO in ITPSMTabBarCell
    }
...
- (BOOL)session:(PTYSession *)session shouldAllowDrag:(id<NSDraggingInfo>)sender {
    if ([[[sender draggingPasteboard] types] indexOfObject:@"com.iterm2.psm.controlitem"] != NSNotFound) {
        // Dragging a tab handle. Source is a ITPSMTabBarControl.
        PTYTab *theTab = (PTYTab *)[[[[ITPSMTabDragAssistant sharedDragAssistant] draggedCell] representedObject] identifier];
