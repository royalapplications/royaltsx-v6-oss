...
- (void)highlightRun:(VT100GridRun)run
    ...
    // iTermLib Edit: Use sRGB color space instead of generic RGB color space. Otherwise, it can happen that the colors configured in the terminal don't match the ones in triggers. To reproduce: Use a non-100% black BG color and use the same BG color in a trigger. They will look different with the original code.
    // Note that this still happens in official iTerm2 as of Version 3.4.15
    NSColor *genericFgColor = [fgColor colorUsingColorSpace:NSColorSpace.sRGBColorSpace];
    NSColor *genericBgColor = [bgColor colorUsingColorSpace:NSColorSpace.sRGBColorSpace];
