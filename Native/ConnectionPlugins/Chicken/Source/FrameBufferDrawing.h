...
- (void)drawRect:(NSRect)aRect at:(NSPoint)aPoint
{
    ...
    if ((aRect.size.width * aRect.size.height) > SCRATCHPAD_SIZE || YES) {
        ...
        if (pixels) {
            ...
            BOOL TODO_ERROR_CGDataProvider_BufferIsNotBigEnough_WORKAROUND = NO;
            
            if (@available(macOS 10.13.5, *)) {
                TODO_ERROR_CGDataProvider_BufferIsNotBigEnough_WORKAROUND = YES;
                
                //NSLog(@"TODO_ERROR_CGDataProvider_BufferIsNotBigEnough_WORKAROUND=YES");
            }
            
            if (TODO_ERROR_CGDataProvider_BufferIsNotBigEnough_WORKAROUND) {
