...
- (void)readData:(NSNotification*)aNotification
{
    ...
    do {
        ...
        while(length) {
            consumed = [currentReader readBytes:bytes length:(unsigned)length];
...
{
	NSPasteboardType theType;
	NSString *str;
	
	NSArray<NSPasteboardType>* types = @[ NSPasteboardTypeString, NSPasteboardTypeFileURL ];
...
- (void)sendPasteboardToServer:(NSPasteboard *)pb automaticallyAllowingLossyConversion:(BOOL)automaticallyAllowLossyConversion
{
    ...
    if (cStr == NULL) {
        if (automaticallyAllowLossyConversion) {
			...
			alert.messageText = ChickenVncFrameworkLocalizedString(@"PasteConversionHeader", nil);
			alert.informativeText = ChickenVncFrameworkLocalizedString(@"PasteConversionBody", nil);
			
			[alert addButtonWithTitle:ChickenVncFrameworkLocalizedString(@"PasteAnyways", nil)];
			[alert addButtonWithTitle:ChickenVncFrameworkLocalizedString(@"Cancel", nil)];
			
			[str retain];
			
			__weak RFBConnection* weakSelf = self;
        ...
        [self sendStringToServersClipboard:cStr length:(unsigned)strlen(cStr)];
    }
}
